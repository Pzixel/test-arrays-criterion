...
fn criterion_benchmark(c: &mut Criterion) {
    ...
    let arrs: Vec<_> = (0..SAMPLES)
        .map(|_| {
            let rng = rand::thread_rng();
            rng.sample_iter(Uniform::new(i32::min_value(), i32::max_value()))
                .take(TOTAL_NUMS)
                .collect::<Vec<_>>()
        })
        .collect();
    ...
    for (i, nums) in arrs.iter().enumerate() {
        ...
        let searches: Vec<_> = (0..SEARCHES_COUNT)
            .map(|_| nums[rng.gen_range(0..nums.len())])
            .collect();
        ...
        group.bench_with_input(
            BenchmarkId::new("Binary", i),
            &(sorted_nums.clone(), searches.clone()),
        ...
        group.bench_with_input(
            BenchmarkId::new("Unsorted", i),
            &(nums.clone(), searches.clone()),
            |b, (nums, searches)| b.iter(|| linear_search(&nums, searches.as_ref())),
        );
    }
    group.finish();
