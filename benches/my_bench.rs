...
fn criterion_benchmark(c: &mut Criterion) {
    ...
    for (i, nums) in arrs.iter().enumerate() {
        let sorted_nums: Vec<_> = nums.iter().sorted().collect();
        ...
        group.bench_with_input(BenchmarkId::new("Binary", i), &(&sorted_nums, &searches),
                               |b, (nums, searches)| b.iter(|| {
                                   let mut counter = 0;
        ...
        group.bench_with_input(BenchmarkId::new("Sorted", i), &(&sorted_nums, &searches),
                               |b, (nums, searches)| b.iter(|| {
                                   let mut counter = 0;
                                   for i in searches.iter() {
                                       for x in nums.iter() {
        ...
        group.bench_with_input(BenchmarkId::new("Unsorted", i), &(nums, &searches),
                               |b, (nums, searches)| b.iter(|| {
                                   let mut counter = 0;
                                   for i in searches.iter() {
                                       for x in nums.iter() {
